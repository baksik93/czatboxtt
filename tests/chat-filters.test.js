...
test('published assets force a fresh filter script and service worker cache', () => {
  ...
  const hotfix = fs.readFileSync(path.resolve(__dirname, '../web-client/public/app-hotfix-v145.js'), 'utf8');
  assert.match(html, /app-hotfix-v145\.js\?v=145/);
  assert.doesNotMatch(html, /src="\/app\.js/);
  assert.equal(hotfix, app);
  assert.match(app, /serviceWorker\.register\('\/sw\.js\?v=145'\)/);
  assert.match(worker, /czatbox-ttm-v145/);
  assert.match(worker, /app-hotfix-v145\.js\?v=145/);
});
...
test('installed desktop cannot replace the uniquely named remote hotfix with its bundled app.js', () => {
  ...
  assert.doesNotMatch(main, /app-hotfix-v145/);
});
